...
const LoginBtn = () => {
  const { loggedInUser, signOut } = useCart();
  ...
  const userMenuRef = useDetectClickOutside({ onTriggered: () => setShowMenu(false) })

  return (
    <div ref={userMenuRef}>
      {!loggedInUser ? <button className="bg-orange-500 hover:bg-orange-700 cursor-pointer text-white font-bold rounded-full py-2 px-6 select-none" onClick={() => navigate('/login')}>
        Login
      </button> : 
      <div className="relative cursor-pointer hover:bg-slate-300 rounded-full dark:text-white dark:hover:text-black">
        ...
        <div className="w-max bg-secondary-color ring-2 ring-orange-300 font-semibold text-center rounded absolute mt-3 -right-20 z-10 flex flex-col gap-y-2 p-2 dark:bg-slate-900 dark:text-white user-menu">
          ...
          <h2>Signed in as:<br /> <span className="text-sm">{loggedInUser.email}</span></h2>
          <h1 
          ...
          <button className="bg-orange-500 hover:bg-orange-700  text-white font-bold rounded-full py-2 px-6 select-none" onClick={handleSignOut}>Sign Out</button>
