...
const PreviousOrders = () => {
  const { orders, initialProducts, clearOrders } = useShop();
  ...
  return (
    <div className={`dark:bg-slate-900 h-screen flex flex-col gap-4`}>
      ...
      <div className="flex flex-wrap gap-4 justify-center items-center p-5">
        {noPreviousOrders ? (
          <p className="ml-3 mt-3 text-xl dark:text-white">
            No previous orders to show
          </p>
        ) : (
          orders.map((order, index) => {
            const item = initialProducts.current.find(
            ...
            return (
              <div
                className="h-fit w-80 p-4 grid grid-cols-2 bg-orange-400 rounded ring-1 ring-black"
                key={index}
              >
                <div className="">
                  <h1 className="text-lg font-semibold">{title}</h1>
                  <p className="text-xl font-extrabold">
                    {formatCurrency(price * order.quantity)}
                  </p>
                ...
                <div className="date-and-time col-span-2 mt-3 bg-slate-900 rounded-full text-center ring-1 ring-black text-white">
                  Ordered On{" "}
                  <span className="font-semibold">{order.dateAndTime[0]}</span>{" "}
                  at{" "}
                  <span className="font-semibold">{order.dateAndTime[1]}</span>{" "}
                  <br />
