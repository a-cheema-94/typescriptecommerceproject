      ...
      <div className="border-2 border-neutral-200 flex flex-col justify-center rounded px-4 py-8 bg-secondary-color dark:bg-slate-300 gap-4 h-full">
          ...
          <Link to='/productPage' state={{
            ...
            <img 
                ...
                className="object-scale-down w-full p-3  mix-blend-multiply hover:cursor-pointer" 
                    />  
            <span className="tooltip invisible w-24 bg-neutral-500 text-white font-semibold text-center rounded absolute">View product details</span>        
          </Link>
        </div>
        ...
        <div className="flex flex-col px-5 card-info-contents">
          ...
          <div className="price-wishlistBtn flex gap-x-4 mt-2">
            <p className="font-bold text-gray-900 text-3xl">{formatCurrency(price)}</p>
            <button 
              className="relative wishlist-btn"
              onClick={() => addToWishList(product)}
            >
              <WishListBtn />
              <span className="tooltip invisible w-fit bg-neutral-500 text-white font-semibold text-center rounded absolute">Add to Wish list</span>
            </button>
          </div>
        </div>
        <AddToCartBtn id={id}/>
