...
const Checkout = () => {
  const { cartProducts, initialProducts, getSubTotal, loggedInUser } = useCart();
  ...
  return (
    <div className={`checkout-page bg-secondary-color dark:bg-slate-900 ${isCartEmpty ? 'h-screen' : 'h-full'} flex flex-col gap-y-4`}>
      ...
      <div className="self-center checkout-item flex flex-wrap justify-center gap-5 ml-2 ">
        {!isCartEmpty ? cartProducts.map((item, id) => {
          ...
          return (
            ...
            <div className="w-80 flex flex-col gap-y-4 border-2 border-orange-500 p-5 rounded bg-orange-200 dark:bg-slate-300 mb-6 mx-1" key={id}>
      ...
      {!isCartEmpty && <div className="self-end flex flex-col items-center mr-20 gap-y-3 mb-3">
      
        <p className="font-semibold text-xl dark:text-white">
          TOTAL: <span className="font-bold text-2xl">
          {formatCurrency(getSubTotal())}
          </span>
        </p>

        <button className="w-44 bg-orange-500 hover:bg-orange-700 rounded p-3 font-semibold text-white cursor-pointer select-none" onClick={handleBuyNowBtn}>
          Buy Now
        </button>
      </div>}
